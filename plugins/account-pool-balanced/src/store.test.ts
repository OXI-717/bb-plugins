...
import { AccountStore, PoolAffinityStore, QUOTA_MIGRATIONS, QuotaStore } from "./store.js";
...
describe("session observations", () => {
  it("keeps confirmed routing after affinity expires and isolates hosts", () => {
    const db = new Database(":memory:");
    try {
      for (const migration of QUOTA_MIGRATIONS) db.exec(migration);
      const store = new PoolAffinityStore(db);
      const key = JSON.stringify(["codex", "host-1", "session:abc"]);
      const accountId = "11111111-1111-4111-8111-111111111111";
      store.recordObservation(key, accountId, 100);
      store.loadBindings(1000, 4096);
      expect(new PoolAffinityStore(db).observation(key)).toEqual({ accountId, lastUsedAt: 100 });
      expect(store.observation(JSON.stringify(["codex", "host-2", "session:abc"]))).toBeNull();
      store.recordObservation(key, "22222222-2222-4222-8222-222222222222", 200);
      expect(store.observation(key)?.accountId).toBe("22222222-2222-4222-8222-222222222222");
    } finally { db.close(); }
  });
});
