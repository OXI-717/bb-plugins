...
export class PoolAffinityStore {
  ...
  recordObservation(key: string, accountId: string, now: number): void {
    this.db.prepare(`INSERT INTO pool_observation (affinity_key, account_id, last_used_at) VALUES (?, ?, ?)
      ON CONFLICT(affinity_key) DO UPDATE SET account_id = excluded.account_id, last_used_at = excluded.last_used_at`).run(key, accountId, now);
    this.db.prepare(`DELETE FROM pool_observation WHERE affinity_key NOT IN (SELECT affinity_key FROM pool_observation ORDER BY last_used_at DESC LIMIT 4096)`).run();
  }

  observation(key: string): AccountBinding | null {
    const row = this.db.prepare("SELECT * FROM pool_observation WHERE affinity_key = ?").get(key);
    if (row === undefined) return null;
    const parsed = affinityRowSchema.parse(row);
