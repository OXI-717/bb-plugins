...
The local CLI and ACP paths were verified with a single PAT and SWE-2 high. Requests from one CLI process are pinned to one pooled account, including under balanced routing; a new CLI process can select a different account after an HTTP quota rejection. Cross-account failover during an existing Devin conversation is deliberately avoided because the CLI can cache an account-specific JWT, and a live two-PAT switch has not been verified. Devin currently exposes no quota telemetry to this adapter, so the pool advances on upstream quota rejection rather than a displayed percentage.
